...
public class Arrow extends Group {
    ...
    private final double ARROW_SCALER = 5;
    private final double ARROWHEAD_ANGLE = Math.toRadians(10);
    ...
    private void update() {
        ...
        //arrowhead 1
        double v = Math.cos(theta + ARROWHEAD_ANGLE) * ARROWHEAD_LENGTH;
        double x = x1 + v;
        double v1 = Math.sin(theta + ARROWHEAD_ANGLE) * ARROWHEAD_LENGTH;
        double y = y1 + v1;
        headA.getPoints().setAll(x, y, x1, y1);
        double v2 = Math.cos(theta - ARROWHEAD_ANGLE) * ARROWHEAD_LENGTH;
        x = x1 + v2;
        double v3 = Math.sin(theta - ARROWHEAD_ANGLE) * ARROWHEAD_LENGTH;
        y = y1 + v3;
